...
namespace SMPMenuGen
{
  public partial class GenerateMenu
  {
    ...
    private void generateXML(menuType direction)
    {
      ...
      {
        menuPos = "menuYPos:" + menudef.menuPos;
        skeletonFile = "StreamedMPEditor.xmlFiles.HBasicHomeSkeleton.xml";
      }
      else
      {
        menuPos = "menuXPos:" + menudef.menuPos;
        skeletonFile = "StreamedMPEditor.xmlFiles.VBasicHomeSkeleton.xml";
      }

      if (helper.getInfoServiceVersion().CompareTo(isWeatherVersion) >= 0)
      ...
      if (fanartHandlerUsed)
      {
        if (fanartHandlerRelease2)
        {
          xml = xml.Replace("<!-- BEGIN GENERATED DEFINITIONS -->"
                        , "<define>#menuitemFocus:" + menudef.focusAlpha + menudef.focusColor + "</define>"
                        + "<define>#menuitemNoFocus:" + menudef.noFocusAlpha + menudef.noFocusColor + "</define>"
                        + "<define>#labelFont:" + menudef.labelFont + "</define>"
                        + "<define>#selectedFont:" + menudef.selectedFont + "</define>"
